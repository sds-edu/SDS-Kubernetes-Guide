...
<p align="center">
  ...
  <br>
  Image source: <a href="https://www.simform.com/blog/kubernetes-architecture">Simform - Kubernetes Architecture Blog</a>
</p>
...
View cluster details by running:
...
To see all nodes that can be used to host the applications, run:
...
Run the command below to see the deployment you just created:
...
Here we can see the details about the pods including the containers running inside the pod. Like IP address, image, container ID, etc.
...
This is because of networking restrictions in Docker Desktop. You can't directly reach pods from your host by default. To overcome this, execute
...
> To stop the command and return to your prompt, press `Ctrl + C`. You can then press the Up Arrow key to quickly bring the command back and run it again.
...
Open [Docker Hub](https://hub.docker.com/) in your browser. Create a repository in Docker Hub called `test-web-app`. You can do so by clicking on the `Create Repository` button in the `Repositories` tab.

![Docker Hub](images/DockerHubRepo.png)

<sup> Figure 2.1.1 Once you click on the `Create` button, you will be brought to this page. </sup>

Connect your device to Docker Hub by running the following command in your terminal and follow the instructions to log in:
...
This will start a local Kubernetes cluster.
...
After sometime run the command to get the deployments and pods:
